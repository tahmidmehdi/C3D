...
if (window != "genome") {
    ...
    rightOverlaps <- findOverlaps(
        flank(anchor.bed, window, start = FALSE),
        ref.bed
    )
}
...
corMethodUpper <- paste(
    toupper(substr(corMethod, 1, 1)),
    substr(corMethod, 2, nchar(corMethod)),
    sep = ""
)
resultsHeader <- paste(
    "COORD_1\tCOORD_2\tR_",
...
for (r in 1:length(anchor.bed)) { # iterate through promoters
    ...
    regionsInPromoters <- subjectHits(
        promoterOverlaps[which(queryHits(promoterOverlaps) == r)]
    )

    if (window == "genome") { # search the entire genome
        candidateRegions <- setdiff(1:length(ref.bed), regionsInPromoters)
    } else {
        regionsInLeft <- subjectHits(
            leftOverlaps[which(queryHits(leftOverlaps) == r)]
        )
        regionsInRight <- subjectHits(
            rightOverlaps[which(queryHits(rightOverlaps) == r)]
        )
        # regions in window
    ...
    if (length(regionsInPromoters) == 0 || length(candidateRegions) == 0) {
        cat(
            '\rCalculating Correlations: Processed anchor',
            r,
            'of',
            length(anchor.bed)
        )
    ...
    regionIndices <- cbind(
        rep(regionsInPromoters,each = length(candidateRegions)),
    ...
    interactionCandidates <- data.frame(
        COORD_1,
        COORD_2,
        Correlation,
        ID,
...
# filter genomeStats
genomeStats <- genomeStats[with(
    genomeStats,
    Correlation >= rcut & p_Value <= pcut & q_Value<=qcut)
    , ]
cat("\n")

# get ref.bed indices corresponding to COORD_1 & COORD_2
regionIndices.Coord1 <- sapply(
    genomeStats$COORD_1,
    function(x) which(regionNames == x)
)
regionIndices.Coord2 <- sapply(
    genomeStats$COORD_2,
    function(x) which(regionNames == x)
)
...
figureWidth <- as.numeric(commaSepStr_to_vector(
    figureWidth,
    length(anchor.bed)
))
...
if (figures == "y" && length(regionIndices.Coord1) > 0) {
    ...
    interactions.bedpe <- data.frame(
        ...
        start2,
        end2,
        score = genomeStats$Correlation,
        q_Value = genomeStats$q_Value,
        color
    )
    ...
    pdf(
        file = paste(outDir, "/figures_", date, ".pdf", sep = ""),
        height = 11,
        width = 8.5
    )
    ...
    for (r in 1:length(anchor.bed)) { # make figure for each anchor
        ...
        if (r %in% noZoom) { # figure with no zoom
            ...
            intLandscape = plotBedpe(
                ...
                lwdrange = c(0, 2),
                plottype = "loops",
                color = interactions.bedpe$color,
                ymax = 1 / maxCor
            )
            labelgenome(
                chrom,
            ...
            anchorTrack = plotBed(
                ...
                chromend = chromend,
                rownumber = anchor.df$row,
                type = "region",
                color = anchor.df$color,
                row = "given",
                rowlabels = c("Anchors"),
                rowlabelcol = "black",
                rowlabelcex = 1.25
        ...
        mtext(
            paste("Figure ", r, ": Interaction Landscape of ", ids[r], sep = ""),
            side = 1
        )
...
if (tracks == "y") {
    ...
    for (r in 1:length(anchor.bed)) {
        if (is.null(anchorStats[[r]])) {
            cat(
                '\rGenerating Tracks: Created track',
                r,
                'of',
                length(anchor.bed)
            )
        ...
        browserHeader <- paste(
            "browser position ",
            as.character(seqnames(anchor.bed[r])),
            ":",
            start(anchor.bed[r]) - figureWidth[r],
            "-",
            end(anchor.bed[r]) + figureWidth[r],
        ...
        trackHeader <- paste(
            "track type=bedGraph name=\"Anchor\" description=\"",
            gsub("_.*", "", ids[r]),
            "\" visibility=full color=0,0,0 altColor=0,0,0 viewLimits=0:1 autoScale=off gridDefault=on db=",
            assembly,
            sep = ""
        )
        ...
        trackHeader <- paste(
            ...
            sampleName,
            "\" description=\" \" visibility=full color=",
            getRGB(trackNumber, trackPal),
            " altColor=0,0,0 viewLimits=0:1 autoScale=off gridDefault=on yLineMark=",
            rcut,
            " yLineOnOff=on db=",
            assembly,
            sep = ""
        ...
        tmp.df <- data.frame(
            distal = anchorStats[[r]]$COORD_2,
            corr = anchorStats[[r]]$Correlation
        )
