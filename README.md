...
|-----------|-------------|
| `anchor` | BED file of anchor regions. |
...
#### Recommended anchor file format

Do not include the header in the `anchor` file.
...
1. The BED file `test_anchors.bed` contains anchors for ABL1 and BCR. This file was generated by
...
Edit `config.txt` by setting `anchor`, `outDirectory`, and `matrix` with appropriate paths and file names. Run C3D with via `sh c3d config.txt`
